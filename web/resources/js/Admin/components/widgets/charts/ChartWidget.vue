  ...
  <div class="card card-flush" :class="className">
    ...
    <div class="card-header pt-5">
      ...
      <h3 class="card-title align-items-start flex-column">
        <span class="card-label fw-bold text-gray-900">{{title}}
            <KTIcon v-if="growthArrow"
                    :icon-name="growth ? 'arrow-up' : 'arrow-down'"
                    :icon-class="growth ? 'fs-3 text-success' : 'fs-3 text-danger'"
            />
        </span>
...
export default defineComponent({
  ...
  props: {
    ...
    initialData: { type: Object as () => Array<Object>, required: true },
    growthArrow: { type: Boolean, default: false },
  },
  emits: ['setChartData'],
  setup(props, { emit }) {
    ...
    onMounted(() => {
      ...
      emit('setChartData', props.initialData);
    });
    onUpdated(() => {
        emit('setChartData', props.initialData);
    });
    ...
    const growth = computed(() => {
        const data = props.chartData[0].data;
        if (data.length < 2) {
            return false;
        }
        return data[data.length - 1].y > data[data.length - 2].y;
    });
